...
# USDC.sol
out=$(forge create ./src/USDC.sol:USDC --rpc-url "${RPC_URL:=sapphire-localnet}" --private-key $PRIVATE_KEY --legacy --constructor-args $OWNER | grep "Deployed to:" | tr -d '[:space:]')
USDC_ADDRESS=$(echo $out | cut -d':' -f2-)
echo "usdc contract address: $USDC_ADDRESS"

# GovernanceToken.sol
out=$(forge create ./src/GovernanceToken.sol:GovernanceToken --rpc-url "${RPC_URL:=sapphire-localnet}" --private-key $PRIVATE_KEY --legacy --constructor-args $OWNER $USDC_ADDRESS | grep "Deployed to:" | tr -d '[:space:]')
